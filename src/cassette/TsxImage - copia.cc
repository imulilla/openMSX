...
TsxImage::TsxImage(const Filename& filename, FilePool& filePool, CliComm& cliComm,std::string IPSI)
{
	setFirstFileType(CassetteImage::UNKNOWN);
	convert(filename, filePool, cliComm,IPSI);
}
...
void TsxImage::convert(const Filename& filename, FilePool& filePool, CliComm& cliComm, std::string IPSI)
{
	File file(filename);
	size_t size;
	auto buf = file.mmap();
#ifdef DEBUG
	cliComm.printInfo(IPSI);
#endif
 		...
 		while (pos < size) {
			...
			if (bid == B32_ARCHIVE_INFO) {
				...
				if (bid == B35_CUSTOM_INFO) {
					...
					{
#ifdef DEBUG
						cliComm.printInfo(char(&buf[pos+6])+char(&buf[pos+7]));
#endif
						if (!memcmp(&buf[pos + 6], "CURSORES", 8))
						{
							unsigned int ipspos;
							unsigned int bloque;
